"""Deploy PredictionMarket contract to GenLayer Bradbury testnet"""
import json
import os
...
def main():
    ...
    with open("prediction_market.py") as f:
        source = f.read()
    ...
    contract = Contract.deploy(
        ...
        sender=account,
        source=source,
        contract_class="PredictionMarket",
        constructor_args=[],
    )
    ...
    with open("market_deploy.json", "w") as f:
        json.dump(info, f, indent=2)
    print(f"Saved to market_deploy.json: {json.dumps(info)}")
